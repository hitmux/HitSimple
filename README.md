...
The extension source is in `vscode/hitsimple/`. It provides syntax highlighting, indentation, snippets, the `$hsc` Problem Matcher, and Build/Run/Debug Current File commands. Debug Current File rebuilds the active `.hs` entry with one `-g` flag, then starts GDB through the Microsoft C/C++ extension's `cppdbg` debugger. It does not create `launch.json` or override F5.
...
VS Code Extension Host tests require `xvfb-run` in headless Linux environments. On Linux x86_64 they install `ms-vscode.cpptools` into an isolated extension directory and require GDB:
