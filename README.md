...
`v0.3.2` is the current Beta release with builds for:
...
Use `hitsimple_0.3.2_arm64.deb` on arm64. The package depends on `clang-18`, or a Debian `clang` package at version 18 or later. When the default Ubuntu 22.04 or Debian 12 repositories do not provide Clang 18, first configure the corresponding `jammy` or `bookworm` repository from [apt.llvm.org](https://apt.llvm.org/); the DEB package does not modify system package sources.
...
```text
hitsimple-0.3.2-macos-arm64.tar.gz
hitsimple-0.3.2-macos-x86_64.tar.gz
```
...
```text
hitsimple-0.3.2-windows-x86_64-full.zip
```
...
```text
hitsimple-0.3.2-windows-x86_64-slim.zip
```
...
code --install-extension dist/hitsimple-vscode-0.3.2.vsix --force
```
