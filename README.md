...
Windows 只生成当前 Windows x64 平台程序，目标 ABI 固定为 `x86_64-w64-windows-gnu`。本轮不包含 RPM、macOS、其他 UNIX，也不提供跨目标 `hsc --target`。

Windows `f128` 通过 Boost 1.85.0 `cpp_bin_float` 的 113-bit 软件后端实现，在 runtime 边界按原始 IEEE 754 binary128 位模式传递。十进制转换和 binary128 编码显式执行 `roundTiesToEven`；NaN payload 的保留与传播仍属于实现定义行为。Linux 继续使用原生 LLVM `fp128` 与 glibc 接口。
...
- C compatibility 是受限子集。C struct 传值 ABI 仅支持已覆盖的 x86_64 SysV ELF 布局；Windows 会明确拒绝。
- `f16` 数学使用 `f32` 计算后回写。Linux `f128` 依赖平台的 binary128/glibc 支持。
