...
`v0.3.4` is the current Beta release with builds for:
...
```text
hitsimple-0.3.4-macos-arm64.tar.gz
...
```text
hitsimple-0.3.4-windows-x86_64-full.zip
```
...
```text
hitsimple-0.3.4-windows-x86_64-slim.zip
```
...
code --install-extension dist/hitsimple-vscode-0.3.4.vsix --force
```
