...
```bash
sudo apt install ./hitsimple_0.3.2_amd64.deb
```

On arm64, use:

```bash
sudo apt install ./hitsimple_0.3.2_arm64.deb
```
...
```bash
sudo dnf install ./hitsimple-0.3.2-1.fc44.x86_64.rpm
sudo dnf install ./hitsimple-0.3.2-1.el9.aarch64.rpm
```
...
```bash
tar -xzf hitsimple-0.3.2-macos-arm64.tar.gz
cd hitsimple-0.3.2-macos-arm64
bin/hsc --clang /opt/homebrew/opt/llvm@18/bin/clang path/to/hello.hs -o hello
./hello
```

Use `hitsimple-0.3.2-macos-x86_64.tar.gz` on Intel Macs. The extracted directory can be moved; `hsc` discovers the preprocessor, standard library, and static runtime relative to itself. macOS requires an external Clang 18 or later. The package is unsigned and not notarized, and no PKG is provided.
...
```powershell
Expand-Archive .\hitsimple-0.3.2-windows-x86_64-full.zip
cd .\hitsimple-0.3.2-windows-x86_64-full
