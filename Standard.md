...
Beta.21 延续解释模板作为默认语义承载单元，并收紧标准库函数签名、`impl op` 匹配、C 兼容层翻译、诊断矩阵和标准模板库规范性定义。标准模板库提供基础模板；编译器可以为标准模板使用内建 fast path，但可观察行为必须与标准模板库语义一致。

### 1.1 规范用语

- **必须**：实现和程序均需满足的强制要求。
- **不得**：被禁止的行为。
- **应当**：推荐行为；偏离时实现或程序应有明确理由。
- **可以**：允许行为。
...
1. 标准库函数统一采用“签名 + View 要求 + 边界规则 + 失败行为”的描述方式。
2. `impl op` 解析改为候选集、适用性、冲突和返回 View 四阶段模型。
3. `op =` 的第一个参数明确为赋值目标的可写左值 View 借用。
4. 标准整数模板名称中的数字明确表示 bit 宽度。
5. `set` 的持久模板目标收紧为可静态定位的名称或成员链，并定义成员链覆盖优先级。
6. C 兼容层增加最小语法、翻译表和核心语义约束。
7. `mut self` 保持预留；Beta.21 采用显式地址或赋值目标写入方案。
8. 核心语句终止符支持换行和分号双写法。
9. 必须诊断项扩展，并提供非规范性一致性测试矩阵。
10. 标准模板库伪代码扩展到 `i32`、`f64`、`bool`、`addr`、`handle`、`cstr`、`bytes` 的最小规范行为。
11. 执行模式扩展为 `unchecked`、`static-checked` 和 `checked` 三档安全策略。
...
1. **unchecked 模式**：实现不要求插入安全检查。越界访问、除零、非法解引用、重复释放、释放后访问等按对应条款处理，通常属于未定义行为。
...
标识符只能包含 `A-Z`、`a-z`、`0-9` 和 `_`，且不得以数字开头。标识符区分大小写。单独 `_` 是多重赋值左侧的丢弃目标，不得作为声明名、参数名、函数名、模板名、成员名或普通引用名称。形如 `t` 后接一个或多个数字的名称（例如 `t0`、`t10`）保留给模板实例计数字面量，不得作为普通标识符。以下划线开头且长度大于 1 的标识符建议预留给实现。
...
| 关键字 | 预期用途 |
|---|---|
| `mut` | 可写方法参数预留；Beta.21 中出现时按诊断项处理 |
...
`self` 是方法声明中的保留名称；方法的第一个参数可以写为 `self as Template`。标准不提供隐式 `self`。Beta.21 中可写行为通过赋值目标、`op =` 的目标借用或显式地址解引用表达。
...
`alloc()`、`calloc()`、`realloc()` 和 `free()` 管理动态存储对象。动态对象没有语言级名称；程序通过地址值访问。动态存储对象的边界元数据在 checked 模式下应由运行时跟踪；static-checked 模式不得为了动态对象边界安全插入运行时元数据维护。
...
`set target as Template` 修改目标名称或成员链的持久默认解释模板；`set target as none` 将目标名称或成员链的持久默认解释模板设置为 `none`。Beta.21 中 `target` 只能是可静态定位的名称或成员链：
...
设置固定长度模板时，目标长度必须匹配模板长度。设置用户模板时，目标长度必须等于 `sizeof(Template)`。对索引、切片、解引用、动态长度 View 或临时表达式使用 `set` 必须产生编译诊断。动态视图需要临时解释时，应使用表达式级 `as`。
...
`resize_bytes()` 按地址顺序复制字节；在大端平台上，低地址前缀不等于整数最低有效字节。需要数值扩展或截断时，应使用整数赋值语义、显式整数赋值 `%d=` 或 `to_iN()` / `to_uN()`。
...
标准模板库定义 `iN`、`uN`、`fN`、`bool`、`addr`、`cstr`、`bytes` 等模板的默认操作。普通 `+`、`-`、`==` 等在操作数携带标准模板时解析到标准模板库操作。标准模板库还定义标准模板的一元 `-` 和 `~` 的内建语义；逻辑 `!`、`&&`、`||` 始终由 8.8 的布尔判断路径处理。这些一元操作在 Beta.21 中不开放给用户模板重载。

标准库允许声明混合标准模板操作；每个组合必须以规范签名列入标准模板库。Beta.21 可移植核心只保证同模板二元运算；跨宽度、跨有符号性以及整数/浮点混合运算属于实现列明的标准库扩展，未列明组合必须产生编译诊断。

### 8.6 用户模板 `op`

用户模板可在 `impl` 中定义默认操作。匹配要求按 8.2 执行。用户模板之间、用户模板与标准模板之间的混合操作必须显式定义。标准不进行隐式跨模板转换。

用户 `op` 返回签名不得为 `-> ()`。`op format` 和未来标准扩展可定义专门 ABI，但仍必须提供静态返回 View。
...
这些操作均按静态模板匹配解析。除 `format` 外，用户模板可重载操作在 Beta.21 中均为二元操作，形参数量必须恰好为 2；`op =` 的两个参数分别是赋值目标和源 View。`op format` 固定为 `op format (value as Template, out[P] as addr) -> i32`。`format` 是 `op` 声明中的上下文操作名，其他位置按普通标识符解析。Beta.21 暂不开放用户模板重载一元 `!`、`~`、一元 `-` 或 `++`/`--`；标准模板库仍可为这些内建运算定义规范语义。操作参数必须显式携带模板。返回签名必须静态确定长度和模板。
...
`op =` 的第一个参数表示赋值目标，必须以可写左值 View 借用方式传入。标准推荐形参名为 `dst`。操作体可以写入目标成员；返回签名用于校验操作体 `return` 表达式，必须静态确定，并且必须与第一个参数的模板和长度一致。赋值表达式的可观察结果固定为赋值目标当前字节形成的右值副本。
...
方法的第一个参数必须显式声明，且其模板必须等于所在 `impl TemplateName` 的模板；零参数方法必须产生编译诊断。推荐写作 `self as Template`。方法内部没有额外隐藏权限，字段访问仍遵循普通成员访问规则。
...
`mut` 在 Beta.21 中仍为预留关键字。实现遇到 `mut` 方法参数必须产生编译诊断，除非实现文档明确启用实验扩展。

Beta.21 中模板方法的 `self` 按 View 值复制。需要写入外部对象时，程序必须使用以下显式方案之一：
...
可移植程序应只依赖本节列出的同模板整数操作和实现明确列入标准库签名表的混合操作。
...
`addr` 长度为 `P`。默认赋值可以接收 `addr`、整数字面量、携带整数解释属性的 `none` View、`iN`、`uN` 和 `bool`，并按地址值复制或按整数规则调整到 `P` 字节。`addr` 支持相等比较和格式化。地址算术不通过普通 `addr + integer` 隐式执行；程序应使用 `?` 取得无符号整数视图，并通过 `addr` 目标赋值接收结果。
...
`lvalue++` 和 `lvalue--` 是语句。带整数标准模板的左值按对应模板整数语义执行；模板为 `none` 的左值按当前长度执行整数加一或减一。对 `fN`、`bool`、`addr`、`cstr`、`bytes` 或用户模板左值使用 `++`、`--` 必须产生编译诊断，除非对应模板在未来版本显式定义自增自减操作。
...
`throw expression` 将表达式结果赋值给最近的动态 `catch` 参数。`catch` 参数是局部存储对象，可带模板，长度按普通局部声明规则静态确定：固定长度标准模板和用户模板可推断长度；`bytes`、`cstr`、`none` 或省略模板时必须提供显式静态长度。长度无法静态确定时必须产生编译诊断。赋值失败按普通赋值诊断或运行时错误处理。当前语法每个 `try` 只带一个 `catch`；嵌套 `try` 按动态最近原则匹配。传播 `throw` 时，离开作用域的局部 `new` 对象按 5.5 释放。没有匹配 `catch` 时，程序以实现定义的错误状态终止，并按实现文档执行运行时资源释放。unchecked 和 static-checked 模式下运行时未定义行为不会自动转换为 `throw`；checked 模式下已检测到的运行时错误可以按实现文档转化为 `throw`。
...
| `fseek` | `fseek(fh as handle, offset as i64, whence as i32) -> i32` | 定位文件位置。`whence` 取值由实现文档定义，推荐兼容 C 的 `SEEK_SET/SEEK_CUR/SEEK_END`。 |
...
HitSimple 推荐使用基于 mcpp 的预处理器。预处理指令使用 `$` 前缀。宏展开、条件编译和文件包含语义以 C99/C11 预处理器为基础；实现提供的额外扩展必须文档化。预处理在核心语言解析前完成，预处理器指令可以出现在任意物理行；完成预处理后，核心解析器只接收展开后的核心语言词法单元。
...
引号形式优先搜索当前目录；尖括号形式搜索系统包含路径。官方系统头按上表分组；源文件只包含其实际使用的头文件。搜索路径和最大包含深度属于实现定义行为；推荐最大包含深度至少 200。

### 15.4 预定义宏

实现应提供 `__HS_VERSION__`、`__HS_POINTER_SIZE__`、`__HS_BYTE_ORDER__`、`__HS_EXECUTION_MODE__`、`__HS_STATIC_CHECKED__`、`__HS_CHECKED__` 等预定义宏，并文档化其具体取值。

---

## 16. C 兼容层

C 兼容层是等价语法层。兼容语法进入语义分析前必须翻译为 HitSimple 核心语法，后续行为全部引用核心语义。Beta.21 只标准化最小兼容子集；实现接受更多 C 语法时必须作为扩展文档化。
...
| `p - q` | 两侧必须具有相同元素元数据；翻译为 `(p? - q?) / sizeof(T)`，结果为实现选定的有符号整数模板，推荐 `i64` |
...
由于 `;` 是语句终止符，旧式 `;Template`、`;none` 必须产生编译诊断或在兼容层词法翻译阶段、进入核心语法前显式翻译为 `as Template`、`as none`。旧式 `[sN]` 可以作为 `[tN]` 的兼容别名。`reinterpret()`、`to_float()` 和 `to_int()` 不属于兼容层，必须按第 7.9 节诊断。
...
可变长度模板 `cstr` 和 `bytes` 不得按值出现在核心 `extern` 函数签名中。需要传递 C 字符串或缓冲区时，应使用 `[P] as addr`，并由 ABI 文档说明该地址指向的对象约束。兼容层可以把 `char*` 或 `cstr*` 翻译为 `[P] as addr`。
...
FFI 传入地址、裸地址和文件句柄的边界检查能力属于实现定义行为。文件句柄以 `handle` View 传递，标准语言层定义的行为包括文件 I/O 函数传递与返回、同模板默认赋值、相等比较和格式化。对不可验证地址执行解引用、释放或重绑定时，static-checked 模式只处理可静态证明的错误；checked 模式下实现应尽可能报告错误，并文档化无法检测的情况。
...
15. 隐式跨模板转换。
16. `mut self` 或任意 `mut` 参数在 Beta.21 标准模式中使用。
17. 对右值赋值、取址或自增自减。
18. `&=` 左侧不是单个 `IDENT`。
...
本章规定所有模式通用的必须诊断项、static-checked 模式安全诊断和 checked 模式运行时错误。非规范性一致性测试清单见第 22 章；当测试样例与正文规则发生冲突时，以正文规则为准。
...
33. 标准库旧名 `to_float()`、`to_int()` 和 `reinterpret()` 必须拒绝，诊断中应给出对应的替代名称。
34. 函数实参表达式之间的具体求值顺序。
...
本章为非规范性附录，用于帮助实现者验证本标准要求。当本章测试样例与正文规范发生冲突时，以正文规范为准。每条规则至少应覆盖一个正例和一个反例。以下清单为最低建议。
...
func ret_as_template() -> as f64 {
    return 1.0
}

func ret_raw16(x[16] as bytes) -> [16] as none {
    return x as none
}

func ret_raw16_implicit_none(x[16] as bytes) -> [16] {
    return x as none
}

func ternary_ok(flag as bool, a as i32, b as i32) -> i32 {
    return flag ? a : b
}

func raw_not(x[4] as none) -> [4] {
    return ~x
}
...
func main() -> i32 {
    new semi_a as i32 = 1; new semi_b as i32 = 2;
    semi_a = semi_a + semi_b;
    new a as Vec2
    a.x = 3.0
    a.y = 4.0
    new b as Vec2
    ...
    new c as Vec2 = a + b
    new raw[16] as bytes
    memset(raw, 0, length(raw))
    new len as f64 = (raw as Vec2).length()
    new len2 as u64 = length(raw)
    new buf[16] as cstr = "Kai"
    new n as u64 = strlen(buf)
    new ia as i32 = 1
    new ib as i32 = 2
    new wide as i64 = ia %8d+ ib
    new fp as f64 = to_f64(ia)
    new scan_count as i32
    new scan_value as i32
    scan_count, scan_value = scanf("%d")
    _, scan_value = scanf("%d")

    new pair[t2] as Vec2
    pair[t0].x = 5.0

    new null_addr as addr = 0
    new ptr as addr = calloc(1, sizeof(addr))
    if (ptr == null_addr) { return 1 }
    [P]*ptr = &ptr
    new ptr_copy as addr = [P]*ptr as addr
    free(ptr)

    new raw2[4] as bytes
    new dyn as addr = alloc(4)
    if (dyn == null_addr) { return 1 }
    set raw2 as i32
    raw2 &= dyn
    raw2 = 7
    free(dyn)
    ...
    new ch as u8 = 'A'
    new truth as bool = raw2

    new static_test as i32 = static_once()
    return 0
}

func static_once() -> i32 {
    static once as i32 = 0
    once = once + 1
    return once
}
```
...
```hs
new x[4] as f64 = 1.0              // 长度不匹配
new a as i32 = 1
new b as f64 = a as f64            // as 长度不匹配
...
impl BadOps {
    op + (lhs, rhs as BadOps) -> BadOps {
        return rhs
    }
}                                  // op 参数缺少模板

impl BadOps {
    op + (lhs as BadOps, rhs as BadOps) -> BadOps { return lhs }
    op + (a as BadOps, b as BadOps) -> BadOps { return b }
}                                  // 重复 op 重载键

impl BadOps {
    op + (lhs as BadOps, rhs as BadOps) -> BadOps { return lhs }
    op + (lhs as BadOps, rhs as BadOps) -> [4] as none { return lhs.x as none }
}                                  // 参数相同但返回不同，仍为重复重载键

new bad_width as i32 = a %0d+ b     // 带类型整数运算宽度为 0
new badf as f64 = a %3f+ b          // 浮点带类型宽度非法

new v[16] as bytes
new x2 = v.length()                // bytes 无 length 方法时诊断

new p as Vec2
new q as Point
new r = p + q                      // 无匹配混合模板操作

func bad(mut self as Vec2) -> Vec2 {
    return self
}                                  // mut 预留，标准模式诊断

set v[0:4] as i32                  // set 目标不是名称或成员链
extern puts(s as cstr) -> i32      // extern 参数按值使用可变长度模板
extern extbuf as bytes             // extern 可变长度变量省略长度

func bad_param(x as bytes) -> () {
    return
}                                  // 普通参数使用可变长度模板但省略长度

func bad_return() -> none {
    return 0
}                                  // 返回模板 none 缺少静态长度

func bad_ternary(flag as bool, a as i32, b as f64) -> i32 {
    return flag ? a : b
}                                  // 三元两个分支模板不同
...
func bad_print_format() -> i32 {
    new x as NoFmt
    return print(x)
}                                  // 用户模板缺少 op format

new arr[t2] as i32                  // [tN] 缺少用户模板

func bad_scan() -> i32 {
    new x as i32
    x = scanf("%d")                 // left-context 必须作为多重赋值唯一右侧
    return 0
}

func bad_scan_discard() -> i32 {
    new count as i32
    count, _ = scanf("%d")          // `_` 只能丢弃转换计数
    return 0
}

func bad_catch() -> i32 {
    try { throw 1 } catch (err as bytes) { return 1 }
    return 0
}                                  // catch 可变长度模板缺少静态长度

impl BadOps {
    func bad_zero() -> i32 { return 0 }
}                                  // 方法缺少显式第一个参数

func bad_scan_zero() -> i32 {
    new count as i32
    count = scanf("literal only")      // left-context 不能用于零赋值转换项，也不能作为普通赋值右侧
    return count
}

func bad_free_interior() -> i32 {
    new p as addr = alloc(16)
    new q as addr = p? + 1
    free(q)
    free(p)
    return 0
...
```c
void *p
p + i                              // void 指针缺少元素大小，必须诊断
typedef missing_t myint            // 未知 typedef 基础类型，必须诊断
void x                              // void 对象，必须诊断
void a[4]                           // void 数组，必须诊断
int (*pa)[4]                        // 指向数组的指针不属于最小兼容子集
p->m                                // p 缺少 struct 元素元数据时必须诊断
```

### 22.3 标准库边界测试

- `memcpy(dst, src, 0)` 不访问源或目标内容，允许零长度内存操作数不解引用。
- `memcpy()` 源目标重叠在 unchecked 模式下为未定义行为，static-checked 模式可静态证明时产生编译诊断，checked 模式可检测时报错。
- `memcpy(dst, src, len)` 在 `len > length(src)` 或 `len > length(dst)` 时，static-checked 模式可静态证明时产生编译诊断，checked 模式下报错。
- `calloc(count, size)` 的乘法溢出在 static-checked 模式可静态证明时产生编译诊断，checked 模式下报错。
- 本小节中 checked 模式下报错的标准库边界测试，在 static-checked 模式中采用可静态证明子集。
- `strcpy(dst, src)` 在目标容量不足时，checked 模式下报错。
- `strncpy(dst, src, n)` 覆盖 `n == 0`、正常终止和补零路径。
- `strlen(cstr_view)` 在 View 长度内找不到 `0x00` 时，checked 模式下报错。
- `fread(dst, size, count, handle)` 对 `size * count` 溢出按 14 章处理。
- `fwrite(src, size, count, handle)` 在 `size * count > length(src)` 时，checked 模式下报错。
- `memcmp(a, b, len)` 在任一源 View 长度不足时，checked 模式下报错。
- `scanf` 部分成功只写入已完成转换项，并返回完成数量。
- `scanf/fscanf` 字面量格式字符串没有赋值转换项时产生编译诊断；动态格式字符串在 checked 模式下报错。
- `free(ptr + 1)`、`free(&slice)` 和 `realloc(ptr + 1, n)` 按非法释放或非法重分配处理。
- `memcpy(dst_addr, src_addr, dynamic_len)` 中 `dynamic_len > 0` 时，裸地址必须具备可检测边界或先形成可读/可写 View。
- 函数内 `static` 初始化只执行一次；初始化期间递归进入同一声明按执行模式诊断或报错。
- 单字节字符字面量可赋给整数模板；多字节字符字面量作为整数源必须产生诊断。

### 22.4 一致性检查项

- `as` 在全文只改变 View 模板。
- `resize_bytes()` 在全文只改变字节长度。
- `i8/i16/i32/i64` 名称中的数字表示 bit 宽度。
- 标准模板库行为可用普通模板规则解释。
- 编译器内建优化具有可观察行为等价要求。
- `op =` 第一个参数写入赋值目标；赋值表达式返回目标当前字节形成的右值副本。
- `impl op` 候选集和冲突规则可复现。
- `set` 成员链覆盖优先级高于模板成员声明；对象或名称模板只选择成员布局。
- C 兼容层每个标准化语法能翻译为核心语法。
- C 非 `char` 数组按 `N * sizeof(T)` 翻译。
- C 指针算术按元素大小换算为字节地址偏移，指针数组元素按 `[P] as addr` 处理。
- static-checked 和 checked 模式新增检查有诊断条目。
- 标准库函数签名、边界和失败行为可测试。
- 附录 EBNF 与正文语义保持一致。
- 标准模板赋值矩阵优先于普通 `impl op` 精确模板匹配。
- C 兼容层表达式子集能翻译为核心表达式。
- 函数内 `static` 初始化时机、递归进入和生命周期均有测试覆盖。
- `-> as Template`、具名返回项、`[N]` 省略 `as` 的返回模板、三元分支合并、`%d/%f` 默认宽度、`scanf` 左上下文、`_` 计数丢弃、字符字面量整数解释和 `bytes` 字典序均有测试覆盖。
...
要求：分号和换行均可结束简单语句；`for` 头部两个分号只作为循环头分隔符。`else` 与 `catch` 前允许空白和任意数量换行，前一块后写入分号时该复合语句结束。
